...
class UserGroupPermissionsController extends AppController {
    ...
    protected function setSearchConds(&$options) {
        ...
        if (isset($this->request->query['name']) && strlen($this->request->query['name']) > 0) {
            ...
            $options['conditions']['name'] = new MongoRegex("/$name/i");
        }
        if (isset($this->request->query['code']) && strlen($this->request->query['code']) > 0) {
            $code = trim($this->request->query['code']);
            $options['conditions']['code'] = new MongoRegex("/$code/i");
        }
        if (isset($this->request->query['module']) && strlen($this->request->query['module']) > 0) {
            $module = trim($this->request->query['module']);
            $options['conditions']['module'] = new MongoRegex("/$module/i");
        }
    }
