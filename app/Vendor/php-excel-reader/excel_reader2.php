...
class Spreadsheet_Excel_Reader {
	 ...
	 *
	 * @param int $sheet - Sheet number in the excel file
	 * @return array $arr - array containing the excel content
	 */
	function dumptoarray($sheet = 0) {
		...
		for($row=1; $row <= $this->rowcount($sheet); $row++) {
			for($col=1; $col <= $this->colcount($sheet); $col++) {
				if (isset($arr[1][$col])) {
					$arr[$row][strtolower($arr[1][$col])] = htmlentities($this->val($row,$col,$sheet));
				} else {
